...
# Function to handle kernel upgrades
kernel_upgrade() {
    print_install "Checking for kernel upgrades"
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    apt-get install -y linux-aws linux-image-aws
    apt-get upgrade -y
    
    # Check if reboot is needed
    if [ -f /var/run/reboot-required ]; then
        print_install "Kernel upgraded - System will reboot after installation"
    fi
}
...
echo -e " ›TEAM 🅥🅝: HOKAGE LEGEND ${YELLOW}(${NC} 2025 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2

# Handle kernel upgrades first
kernel_upgrade
...
# [Rest of your original script content remains the same...]
# [Include all your existing functions and code from the original script]

function instal(){
    clear
    kernel_upgrade  # Added this at the beginning
    first_setup
    nginx_install
    base_package
    make_folder_xray
    ...
    # Check if reboot is needed for kernel
    if [ -f /var/run/reboot-required ]; then
        echo ""
        echo -e "${RED}System requires reboot to complete kernel upgrade${NC}"
